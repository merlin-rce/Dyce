...
/* ============================================================
   ...
   Two control modes (settings gear): turn the ring like the real
...
function setFont(px, w) { ctx.font = `${w} ${px}px Montserrat, Arial, sans-serif`; }
...
const drawLogo = (cx, cy, a) => drawImg(logoImg, cx, cy, 210, a);
...
// ============================================================
// CONTROLS : power, settings, encoder-drag / mouse-wheel
// ============================================================
...
document.querySelectorAll('input[name="mode"]').forEach(r => r.addEventListener('change', () => {
  ...
  device.classList.toggle('encoder', mode === 'encoder');
  device.classList.toggle('mouse',   mode === 'mouse');
}));

// info popover (replaces the old bottom tips)
info.addEventListener('click', (e) => { e.stopPropagation(); infocard.classList.toggle('hidden'); });
document.addEventListener('click', (e) => { if (!infocard.contains(e.target) && e.target !== info) infocard.classList.add('hidden'); });
